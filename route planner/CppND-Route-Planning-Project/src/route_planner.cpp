...
RoutePlanner::RoutePlanner(RouteModel& model, float start_x, float start_y, float end_x, float end_y) : m_Model(model) {
    // Converting inputs to percentage:
    start_x *= 0.01;
    start_y *= 0.01;
    end_x *= 0.01;
    end_y *= 0.01;
...
// NextNode method used  to sort the open list and return the next node.
// Tips:
...
// - When the search has reached the end_nodeif, use the ConstructFinalPath method to return the final path that was found.
